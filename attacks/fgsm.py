#!/usr/bin/env python3
"""
FGSM (Fast Gradient Sign Method) adversarial training script using torchattacks.FGSM
with EfficientNetB0_GCBAM architecture (Ghost + CBAM).

FGSM is a single-step attack proposed by Goodfellow et al. (2014).
It computes the gradient of the loss w.r.t the input once and perturbs
the image in the gradient sign direction — the fastest adversarial attack.
...
Key differences vs PGD/BIM scripts:
  - Uses FGSM (single step) instead of PGD (multi-step + random start)
    or BIM (multi-step, no random start)
  - FGSM only has one parameter: epsilon (no alpha or steps needed)
  - Fastest attack to generate — significantly lower per-epoch compute cost
  - Weakest of the three attacks; models trained with FGSM may be less robust
    to stronger iterative attacks (known as catastrophic overfitting risk)
  - All other infrastructure identical to PGD/BIM scripts
"""
...
# Configuration
# ─────────────────────────────────────────────────────────────
out_dir     = "model_out_gcbam_fgsm"
config_file = "config_fgsm_gcbam.json"
...
# FGSM parameter — single-step attack, only epsilon needed
# (no alpha or steps: FGSM takes one full step of size epsilon)
fgsm_eps = 8 / 255   # perturbation budget (L-inf ball radius)
...
if os.path.exists(config_file):
    try:
        ...
        fgsm_eps   = cfg.get("epsilon",             fgsm_eps)
...
# ─────────────────────────────────────────────────────────────
# FGSM attack
# Note: torchattacks.FGSM signature — FGSM(model, eps)
#       Single-step, no alpha or steps parameters.
#       Inputs must be in [0,1] — matches our preprocessing.
# ─────────────────────────────────────────────────────────────
attack = FGSM(model, eps=fgsm_eps)
...
# Training Loop
# ─────────────────────────────────────────────────────────────
print(f"Starting FGSM adversarial training at {datetime.now()}")
print(f"Epochs: {epochs}  |  Batch: {batch_size}  |  LR: {lr}")
print(f"FGSM — eps={fgsm_eps:.4f}")
print("(Single-step attack — fastest generation, one gradient computation per sample)\n")
...
for epoch in range(start_epoch_from_ckpt, epochs):
    ...
    for batch_idx, (x_batch, y_batch) in enumerate(pbar):
        ...
        # ── Epochs 1+: adversarial training with FGSM ───────────
        else:
            ...
            try:
                ...
                print(f"Batch FGSM attack failed ({e}); falling back to per-sample...")
                adv_list = []
            ...
            # Train on FGSM adversarial examples only
            optimizer.zero_grad()
            with torch.cuda.amp.autocast(enabled=use_amp):
                outputs = model(x_adv)
                loss    = criterion(outputs, y_batch)
