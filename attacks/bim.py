#!/usr/bin/env python3
"""
BIM (Basic Iterative Method) adversarial training script using torchattacks.BIM
with EfficientNetB0_GCBAM architecture (Ghost + CBAM).

BIM is essentially PGD without random start — also known as I-FGSM
(Iterative Fast Gradient Sign Method), proposed by Kurakin et al. (2016).
...
Key difference vs PGD script:
  - Uses BIM (no random start) instead of PGD (with random start)
  - BIM starts from the clean image every time and iterates deterministically
  - Generally produces smaller, more natural-looking perturbations than PGD
  - All other infrastructure (checkpointing, AMP, TensorBoard, HF loading) identical
"""
...
# Configuration
# ─────────────────────────────────────────────────────────────
out_dir     = "model_out_gcbam_bim"
config_file = "config_bim_gcbam.json"
...
# BIM parameters
# BIM: starts from clean image, no random init → deterministic iterative attack
bim_eps   = 8 / 255   # total perturbation budget (L-inf ball radius)
bim_alpha = 2 / 255   # step size per iteration
bim_steps = 15        # number of iterations (same as PGD for fair comparison)
...
if os.path.exists(config_file):
    try:
        ...
        bim_eps     = cfg.get("epsilon",             bim_eps)
        bim_steps   = cfg.get("k",                   bim_steps)
        bim_alpha   = cfg.get("a",                   bim_alpha)
...
class HFFetalPlanesDataset(Dataset):
    ...
    Inputs are kept in [0,1] — no ImageNet normalization applied.
    """
...
# ─────────────────────────────────────────────────────────────
# BIM attack
# Note: torchattacks.BIM signature — BIM(model, eps, alpha, steps)
#       No random_start parameter (deterministic by design).
#       Inputs must be in [0,1] — matches our preprocessing.
# ─────────────────────────────────────────────────────────────
attack = BIM(model, eps=bim_eps, alpha=bim_alpha, steps=bim_steps)
...
# Training Loop
# ─────────────────────────────────────────────────────────────
print(f"Starting BIM adversarial training at {datetime.now()}")
print(f"Epochs: {epochs}  |  Batch: {batch_size}  |  LR: {lr}")
print(f"BIM — eps={bim_eps:.4f}  alpha={bim_alpha:.4f}  steps={bim_steps}")
print("(No random start — BIM is deterministic)\n")
...
for epoch in range(start_epoch_from_ckpt, epochs):
    ...
    for batch_idx, (x_batch, y_batch) in enumerate(pbar):
        ...
        # ── Epochs 1+: adversarial training with BIM ────────────
        else:
            # Switch to eval for stable BN stats during attack generation
            model.eval()
            try:
                x_adv = attack(x_batch, y_batch)
                x_adv = torch.clamp(x_adv, 0.0, 1.0)
            except RuntimeError as e:
                print(f"Batch BIM attack failed ({e}); falling back to per-sample...")
                adv_list = []
            ...
            # Log natural vs adversarial accuracy
            with torch.no_grad():
                preds_nat = model(x_batch).argmax(dim=1)
            ...
            # Train on BIM adversarial examples only
            optimizer.zero_grad()
            with torch.cuda.amp.autocast(enabled=use_amp):
                outputs = model(x_adv)
                loss    = criterion(outputs, y_batch)
